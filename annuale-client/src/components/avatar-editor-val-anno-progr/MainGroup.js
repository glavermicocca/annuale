...
const MainGroup = props => {
  ...
  const imagePath = props.id !== undefined ? `/IMAGES?id=${props.id}&timestamp${time}` : null
  ...
  const handleSave = async () => {
    ...
    resizeImage(imageSrc, croppedAreaPixels.width, croppedAreaPixels.height, croppedAreaPixels.x, croppedAreaPixels.y, rotation, async croppedImage => {
      ...
      try {
        const url = props.id !== undefined ? `/IMAGES?id=${props.id}&val=${props.val}&anno=${props.anno}&progr=${props.progr}` : `/IMAGES?val=${props.val}&anno=${props.anno}&progr=${props.progr}`
